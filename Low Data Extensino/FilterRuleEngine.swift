...
final class FilterRuleEngine {
    ...
    func setCurrentNetworkTrusted(_ trusted: Bool) {
        self.currentNetworkTrusted = trusted
    }
    ...
    func updateCurrentNetwork(_ network: DetectedNetwork) async {
        ...
        logger.info("Network updated: \(network.displayName) - trusted: \(self.currentNetworkTrusted)")
    }
    ...
    func loadDefaultRules() async {
        ...
        logger.info("Loaded \(self.defaultRules.count) default rules")
    }
    ...
    private func applyRule(_ rule: ProcessRule, process: ProcessIdentity, flow: FlowInfo) -> FilterDecision {
        switch rule.action {
        case .allow:
            return .allow(reason: "Rule: Allow \(rule.processIdentifier)")
        case .block:
            blockCount += 1
            return .block(reason: "Rule: Block \(rule.processIdentifier)")
        case .askUser:
            // For MVP, treat askUser as block on untrusted networks
            if !currentNetworkTrusted {
                blockCount += 1
                return .block(reason: "Rule: Ask User (blocked on untrusted network)")
            } else {
                return .allow(reason: "Rule: Ask User (allowed on trusted network)")
            }
        }
    }
