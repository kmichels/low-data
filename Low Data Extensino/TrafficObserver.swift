...
final class TrafficObserver {
    ...
    init() {
        ...
        logger.info("TrafficObserver initialized with capacity: \(self.maxObservations)")
    }
    ...
    func record(_ observation: TrafficObservation) {
        ...
        if observations.count % 1000 == 0 {
            logger.info("Recorded \(self.observations.count) observations")
        }
    }
    ...
    func getObservationsForProcess(_ processId: String) -> [TrafficObservation] {
        return observations.toArray().filter { $0.process.name == processId || $0.process.bundleId == processId }
    }
    ...
    private func updateProcessStatistics(for observation: TrafficObservation) {
        ...
        let processId = observation.process.bundleId ?? observation.process.name
    ...
    func saveToSharedContainer() async {
        ...
        logger.info("Saving \(self.observations.count) observations to shared container")
